...
export class NginxProvider implements VPSProvider<NginxMetrics> {
  ...
  async collect(session: SSHSession): Promise<NginxMetrics> { const version = await session.execute(nginxCommands.version.build(), nginxCommands.version); const config = await session.execute(nginxCommands.testConfig.build(), nginxCommands.testConfig); const logs = await session.execute(nginxCommands.allLogs.build(), nginxCommands.allLogs); return { version: version.stdout.trim(), running: config.exitCode === 0, configValid: config.exitCode === 0, logs: logs.stdout.split("\n").slice(-1500).reverse() }; }
